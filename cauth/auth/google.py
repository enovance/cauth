...
class GoogleAuthPlugin(oauth2.BaseOAuth2Plugin):
    ...
    def get_user_data(self, token):
        ...
        user_url = "https://www.googleapis.com/plus/v1/people/me?key=%s"
        resp = requests.get(user_url % token)
        if not resp.ok:
            logger.error('%s returned: %s', (user_url % '<REDACTED>', resp))
            raise base.UnauthenticatedError(resp)
        data = resp.json()
        login = data.get('nickname')
        if not login:
            login = (data.get('name', {}).get('givenName', 'John') + '.' +
                     data.get('name', {}).get('familyName', 'Doe')[0])
        name = (data.get('name', {}).get('givenName', 'John') + ' ' +
                data.get('name', {}).get('familyName', 'Doe'))
