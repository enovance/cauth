...
from cauth.backends import githubauth, localauth
...
class TestLoginController(TestCase):
    ...
    def test_check_valid_user(self):
        ret = localauth.check_static_user(self.conf, 'user1', 'userpass')
        self.assertIn('user1@tests.dom', ret)
        self.assertIn('Demo user1', ret)
        with patch('requests.get'):
            ret = localauth.check_static_user(self.conf, 'user1', 'badpass')
    ...
    def test_check_localdb_user(self):
        with patch('requests.get') as g:
            ...
            ret = localauth.check_db_user(self.conf, 'les', 'Wynona')
        ...
        with patch('requests.get') as g:
            ...
            ret = localauth.check_db_user(self.conf, 'bootsy', 'collins')
            self.assertEqual(None, ret)
